...
Indicators (KRIs) in {gsm}. KRIs are metrics that allow users to measure
pre-defined risks and determine the level of observed risk to data
quality and patient safety in a clinical trial. The {gsm} package
implements a standardized data pipeline to facilitate KRI analysis.
Other vignettes provide an overview of this framework
([1](https://gilead-public.github.io/gsm/dev/),
[2](https://gilead-public.github.io/gsm/dev/articles/Cookbook.md)
[3](https://gilead-public.github.io/gsm/dev/articles/DataModel.md),
[4](https://gilead-public.github.io/gsm/dev/articles/DataAnalysis.md),
[5](https://gilead-public.github.io/gsm/dev/articles/DataReporting.md)),
and the statistical methods for this process are described in detail
below.

GSM calculates KRIs by defining a numerator and a denominator for each
metric. Then by default, GSM calculates z-scores using a normal
approximation with adjustment for over-dispersion to assign risk levels.
...
methods. More details are provided below.
...
in [gsm](https://github.com/Gilead-BioStats/gsm) calculates adjusted
z-score for each site as discussed above. The adjusted z-scores are then
used as a scoring metric in
[gsm](https://github.com/Gilead-BioStats/gsm) to flag possible outliers
using the thresholds discussed below.
...
can be customized and specified in the {gsm} functions.
...
For the binary outcome KRIs, an optional method in {gsm} is implemented
...
The function `Analyze_Fisher` in {gsm} utilizes
...
in [gsm](https://github.com/Gilead-BioStats/gsm) utilizes
[`stats::fisher.test()`](https://rdrr.io/r/stats/fisher.test.html) to
generate an estimate of odds ratio as well as p-value using the Fisher’s
exact test with site-level count data. For each site, Fisher’s exact
...
scoring metric in [gsm](https://github.com/Gilead-BioStats/gsm) to flag
...
package and can be customized and specified in the {gsm} functions.
...
For the rate outcome KRIs, an optional method in {gsm} is implemented
...
specified in the {gsm} functions.
