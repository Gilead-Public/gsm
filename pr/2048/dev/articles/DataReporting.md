...
Indicators (KRIs) and thresholds to conduct study-level, country-level
and site-level Risk Based Monitoring for clinical trials.
...
run the full site-level report for all 12 KRIs included in this package.
The focus of this vignette is the reporting workflow, so the output of
the analytics workflow will be briefly discussed, but only in the
context of *inputs* to the reporting workflow.
...
[gsm](https://github.com/Gilead-BioStats/gsm) as `reportingGroups`,
`reportingMetrics`, `reportingResults` and `reportingBounds`. These
sample tables are used throughout the package in examples and
documentation.
...
CTMS data and the mapped `Mapped_SUBJ` data frame created in the Mapping
...
and
[`MakeLongMeta()`](https://gilead-public.github.io/gsm/dev/reference/MakeLongMeta.md).
These small tables are then bound together to create `dfGroups`.
...
#> nStep was not provided. Setting default step to 433.42.
#> Parsed -3,-2,2,3 to numeric vector: -3, -2, 2, 3
#> nStep was not provided. Setting default step to 0.596.
```
...
function to create all relevant charts given the input data.
[`MakeCharts()`](https://gilead-public.github.io/gsm/dev/reference/MakeCharts.md)
is a wrapper around several helper functions that generate each static
...
and the HTML output will be knit from the `Report_KRI.Rmd` template. All
intermediate files from the knitting process will be saved in a
temporary folder.
...
Here, you will see two options for how to run your workflows. The first
is to run the analytics workflow(s), followed by the reporting workflow
...
The second option is to run the `snapshot.yaml` workflow, which takes in
all raw data (including site and study data) and a few extra metadata
arguments at the beginning of the workflow, and the result is a list
with all data frames and charts from both models, as well as a report
html saved to your working directory.
...
- List of all charts and tables (`lCharts`) created from `dfResults`,
  `dfBounds`, `dfMetrics` and `dfGroups` using
  [`MakeCharts()`](https://gilead-public.github.io/gsm/dev/reference/MakeCharts.md)
