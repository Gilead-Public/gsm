...
The [gsm](https://github.com/Gilead-BioStats/gsm) package provides a
standardized data pipeline for conducting study-level Risk Based Quality
Management (RBQM) for clinical trials. There are four main types of data
...
In general, [gsm](https://github.com/Gilead-BioStats/gsm) is designed to
be flexible and customizable, allowing users build custom data pipelines
that support many types of raw study data. As shown below, raw clinical
data is transformed into mapped data, which is then analyzed to
calculate desired metrics. The analysis data is then combined and
formatted for reporting with additional raw data, including CTMS data
and [gsm](https://github.com/Gilead-BioStats/gsm) workflow data, which
...
[gsm](https://github.com/Gilead-BioStats/gsm) is designed to work with a
...
[gsm](https://github.com/Gilead-BioStats/gsm). The only requirement is
that the mapped data is compatible with the analytics pipeline. Data
Mapping transformations can be done using multiple methods including
custom R scripts (e.g., with `dplyr`), SQL queries, or using `gsm`
workflows (e.g. the
`system.file("workflow/1_mapping/AE.yaml", package = "gsm")` file).
Examples of these methods can be found in
[`vignette("Cookbook")`](https://gilead-public.github.io/gsm/dev/articles/Cookbook.md).

## Analysis Data

In [gsm](https://github.com/Gilead-BioStats/gsm) analysis data is used
to capture key metrics associated with the conduct of a clinical trial.
As described in the [gsm](https://github.com/Gilead-BioStats/gsm)
...
1.  **Scripted Analysis**: Run each step of the analysis pipeline
    individually using the functions provided in the
    [gsm](https://github.com/Gilead-BioStats/gsm) package. This approach
    is useful for understanding the data requirements and for debugging.
    See Example 1 in
    [`vignette("Cookbook")`](https://gilead-public.github.io/gsm/dev/articles/Cookbook.md)
...
A rigid Reporting Data framework is provided in {gsm} to allow for
standardized reporting, visualization and meta-analysis that compare
risk profiles across timepoints, and even across multiple studies. The
Reporting Data sets used in {gsm} are:
...
1.  `Reporting_Results`: Summary data; Standardized subset of the
    ...
    [`Summarize()`](https://gilead-public.github.io/gsm/dev/reference/Summarize.md)
    function in the analytics pipeline, followed by the
...
2.  `Reporting_Bounds`: Bounded data; A data.frame containing predicted
    ...
    observed values. Created with the
...
[`MakeWorkflowList()`](https://gilead-public.github.io/gsm/dev/reference/MakeWorkflowList.md)
imports YAML specifications from `workflow/2_metrics`. Each item in
...
- `workflow`: Array defining one or more functions to be executed as
  ...
  - `workflow[]$steps`: specifies all of the steps in the workflow.
    - `workflow[]$steps$name`: name of the
      [gsm](https://github.com/Gilead-BioStats/gsm) function.
...
(`system.file("workflow/2_metrics/kri0001.yaml", package = "gsm")`) is
shown below:
